      ...
      {members.map((m) => (
        <div key={m.key} className={`rfull-member${m.afk ? ' afk' : ''}`}>
          ...
          {m.afk
            ...
            : <div className={`rfm-prog${m.progClass ? ' ' + m.progClass : ''}`}>{m.prog}</div>
          }
        </div>
      ))}
    </div>
...
export default function RoomCard({ room }) {
  ...
  const cardClass   = ['rcard', room.live ? 'live' : '', room.width].filter(Boolean).join(' ');
  ...
  const body = (
    <div className="rcard-body" style={isW3 ? { borderTop: 'none' } : {}}>
      ...
      <div className="rcard-prog-bar">
        <div className={`rcard-prog-fill ${room.progClass}`} style={{ width: `${room.percent}%` }} />
      </div>
